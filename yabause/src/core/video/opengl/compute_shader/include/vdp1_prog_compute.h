...
"  uint y = uint(floor(float(texSize.y-1)*uv.y));\n"
...
" vec4 limit_f = vec4(vec2(usrClip.xy)*upscale, vec2(min(usrClip.zw, sysClip))*upscale);\n"
...
"   if (valid) {\n"
"    ivec2 Pn = P;\n"
"    vec4 pixout = getColoredPixel(pixcmd, float(Pn.x-P0.x+1)/float(P1.x-P0.x+1), Pn, valid);\n"
...
"   if (valid) {\n"
"    ivec2 Pn = P;\n"
"    vec4 pixout = getColoredPixel(pixcmd, float(Pn.y-P0.y+1)/float(P1.y-P0.y+1),Pn, valid);\n"
"    if (valid) imageStore(outSurface,Pn,pixout);\n"
"   }\n"
"   a.z += vector.x;\n"
...
"  }\n"
" }\n"
" bool valid = clip(P1,limit);\n"
" if (valid) {\n"
"  vec4 pixout = getColoredPixel(pixcmd, 1.0, P1, valid);\n"
"  if (valid) imageStore(outSurface,P1,pixout);\n"
" }\n"
"}\n";
