...
"int getLine(int D){\n"
" return (D - (D/upscale.y)*upscale.y);\n"
"}\n"
"int getColumn(ivec2 D){\n"
" return abs(D.x - (upscale.x * int(D.x/upscale.x)));\n"
"}\n"
...
"  ivec2 liftA = ivec2(0);\n"
"  ivec2 liftB = ivec2(0);\n"
"  if (any(notEqual(upscale, ivec2(1)))) {\n"
"   liftA = ivec2(((pixcmd.misc>>2) & 0x3u)-2, ((pixcmd.misc>>4) & 0x3u)-2);\n"
"   liftB = ivec2(((pixcmd.misc>>6) & 0x3u)-2, ((pixcmd.misc>>8) & 0x3u)-2);\n"
"  }\n"
...
"  if (a.x == 0) a.x = 1;\n"
"  if (a.y == 0) a.y = 1;\n"
"  float col = mod(P.x, upscale.x);\n"
"  float lin = mod(P.y, upscale.y);\n"
"  bool pixLiftA = "
"     ((liftA.x != 0) && all(equal(P/upscale,P0 + ivec2(liftA.x, 0))))"
"   ||((liftA.y != 0) && all(equal(P/upscale,P0 + ivec2(0, liftA.y))));\n"
"  if (pixLiftA && (liftA.x == liftA.y)) pixLiftA = a.x*(lin-col)<0;\n"
"  if (pixLiftA && (liftA.x != liftA.y) && (liftA.y > 0)) pixLiftA = a.x*(((upscale.x -1) - col)-lin)<0;\n"
"  if (pixLiftA && (liftA.x != liftA.y) && (liftA.y < 0)) pixLiftA = a.x*(((upscale.x -1) - col)-lin)<0;\n"
"  bool pixLiftB = "
"   ((liftB.x != 0) && all(equal(P/upscale,P1 + ivec2(liftB.x, 0))))"
" ||((liftB.y != 0) && all(equal(P/upscale,P1 + ivec2(0, liftB.y))));\n"
" if (pixLiftB && (liftB.x == liftB.y)) pixLiftB = a.x*(lin-col) > 0;\n"
" if (pixLiftB && (liftB.x != liftB.y) && (liftB.y > 0)) pixLiftB = a.x*(((upscale.x -1) - col)-lin)>0;\n"
" if (pixLiftB && (liftB.x != liftB.y) && (liftB.y < 0)) pixLiftB = a.x*(((upscale.x -1) - col)-lin)>0;\n"
...
"  mat2 transrev = trans;\n"
"  P0 = ivec2(trans*P0*upscale);\n"
"  P1 = ivec2(trans*P1*upscale);\n"
"  ivec2 Pn = ivec2(trans*P);\n"
"  a = sign(P1-P0);\n"
"  if (a.x == 0) a.x = 1;\n"
"  if (a.y == 0) a.y = 1;\n"
"  int veclong = P1.x-P0.x+a.x*upscale.x;\n"
"  bool pixelOnLine = (P1.x == P0.x) || pixLiftA || pixLiftB;\n"
"  float slope = 1.0;\n"
"  int lidx =  abs(getLine(Pn.y-P0.y-(1-a.y)/2*(upscale.y-1)));\n"
"  int cidx =  abs(getLine(Pn.x-P0.x-(1-a.x)/2*(upscale.x-1)));\n"
"  if ((!pixLiftA) && (!pixLiftB)) {\n"
"   if (P.x < min(pixcmd.CMDXA,pixcmd.CMDXB)*upscale.x) continue;\n"
"   if (P.x > max(pixcmd.CMDXA,pixcmd.CMDXB)*upscale.x+(upscale.x - 1)) continue;\n"
"   if (P.y < min(pixcmd.CMDYA,pixcmd.CMDYB)*upscale.y) continue;\n"
"   if (P.y > max(pixcmd.CMDYA,pixcmd.CMDYB)*upscale.y+(upscale.y - 1)) continue;\n"
"  }\n"
"  if (!pixelOnLine) {\n"
"    slope = float((P1.y-P0.y)/upscale.y)/float((P1.x-P0.x)/upscale.x);\n"
"    pixelOnLine = isOnLine(Pn/upscale, P0/upscale, slope);\n"
"  }\n"
"  if (!pixelOnLine) pixelOnLine = (onChange((Pn)/upscale, P0/upscale, slope, a, int(orientation), cidx, lidx));\n"
"  if (!pixelOnLine) pixelOnLine = onChangeOpposite(Pn/upscale, P0/upscale, slope, a, int(orientation), cidx, lidx);\n"
"  if (antialiased && !pixelOnLine) pixelOnLine = (onChangeAliased((Pn)/upscale, P0/upscale, slope, a, int(orientation), cidx, lidx));\n"
"  if (pixelOnLine) {\n"
...
"   if(valid) {\n"
// "    if (liftA.x != 0) pixout.a += (liftA.x)*0.1 + 0.5;\n"
// "    if (pixLiftA || pixLiftB) pixout.b = 1.0;\n"
// "    if (liftA.y != 0) pixout.b += (liftA.y+2)*0.1;\n"
