...
"layout(location = 8) uniform ivec2 u_sysClip;\n"
"layout(location = 9) uniform ivec4 u_usrClip;\n"
...
" ivec4 usrClip = ivec4(vec4(trans*vec2(u_usrClip.xy), trans*vec2(u_usrClip.zw)));\n"
" ivec2 sysClip = ivec2(trans*vec2(u_sysClip.xy));\n"
" ivec2 P = P0;\n"
" ivec2 vector = P1-P0;\n"
...
"  //Draw pixels\n"
"  bool valid = clip(P,usrClip, sysClip, pixcmd.CMDPMOD);\n"
"  if (valid) {\n"
"   ivec2 Pn = P;\n"
...
"   valid = clip(Pn,usrClip, sysClip, pixcmd.CMDPMOD);\n"
"   if ((valid)&&(Pn.x != P1.x)) {\n"
"    float dp = (float(Pn.x-P0.x)+0.5*float(a.x))/float(veclong);\n"
...
" Pn.y += a.y*row;\n"
" //Draw pixels\n"
" bool valid = clip(Pn,usrClip, sysClip, pixcmd.CMDPMOD);\n"
" if (valid) {\n"
"  float dp = (float(Pn.x-P0.x)+0.5*float(a.x))/float(veclong);\n"
"  vec4 pixout = getMeshedPixel(pixcmd, dp, ivec2(transrev*vec2(Pn)), valid);\n"
"  if (valid) imageStore(outSurface,ivec2(transrev*vec2(Pn)),pixout);\n"
